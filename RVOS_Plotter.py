...
def makePlot(sim,linestyle='dashed',numpoints=500,fill=True,save='',fmt='png',hide_True=False,date=False,quiet=False):
	...
	if linestyle =='dashed':
		fitLineStyle,trueLineStyle = 'b--','r--'
	elif linestyle =='solid':
		fitLineStyle,trueLineStyle = 'b-', 'r-'
	else:
		assert False, "linestyle must be set to 'dashed' (default) or 'solid'"

	if fill:
		plt.plot(times, RV_fit, fitLineStyle,label='Fit')
		if not hide_True: plt.plot(times, RV_True,trueLineStyle,label='True')
	else:
		plt.plot(star.obsList, star.RV_fit,fitLineStyle,label='Fit')
		if not hide_True: plt.plot(star.obsList, star.RV_True,trueLineStyle,label='True')
